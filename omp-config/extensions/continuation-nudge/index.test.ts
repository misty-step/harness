...
function makeHarness(entries: BranchEntry[] = [], options: {
	...
	const ctx = {
		...
		modelRegistry: {
			getApiKey: async (_provider: string) => (options.providerAuth === false ? undefined : "test-key"),
		},
		ui: {
...
async function settle(harness: Harness): Promise<void> {
	await harness.listeners.get("agent_end")?.({ willContinue: false }, harness.ctx);
}
...
describe("registration", () => {
	test("registers the settle hook, session_start, and /continuation", () => {
		...
		expect(harness.listeners.has("agent_end")).toBe(true);
...
describe("suppression", () => {
	...
	test("does not treat a continuing agent_end as a settle", async () => {
		process.env.JEV_NUDGE_PROVIDER = "stub-nudge";
		const harness = makeHarness([user("request"), assistant("answer")]);
		await harness.listeners.get("agent_end")?.({ willContinue: true }, harness.ctx);
		expect(harness.messages).toHaveLength(0);
		expect(harness.entries).toHaveLength(0);
		expect(existsSync(join(agentDir, "continuation-nudge.jsonl"))).toBe(false);
	});

	test("nudges on a terminal agent_end even though isIdle() is false during the event", async () => {
		process.env.JEV_NUDGE_PROVIDER = "stub-nudge";
		const harness = makeHarness([user("request"), assistant("answer")], { isIdle: false });
		await settle(harness);
		expect(harness.messages).toHaveLength(1);
		expect(harness.entries[0].data.attempt).toBe(1);
	});
