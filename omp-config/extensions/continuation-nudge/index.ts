/**
 * continuation-nudge — bounded advisory continuation nudge for OMP.
 *
 * When the agent finishes a run, ask Jev (System One via the OpenRouter
 * Decisions API, pinned `typesafe/jev-1.13`) one bounded Choice question:
 * would a gentle nudge help the agent advance useful work within the user's
 * existing request? A confident `nudge` injects the fixed advisory message as
 * a follow-up turn; everything else finishes normally.
 *
 ...
 * answer ends the run exactly like stock OMP. Loop protection is
 ...
 * calling Jev at all.
 *
 * OMP differences from the pi sibling (documented in omp-config/README.md):
 *   - OMP 18.2.6 has no `agent_settled` event. The settle moment is
 *     `agent_end` with `willContinue !== true` (OMP sets `willContinue` when
 *     it will retry or continue automatically), plus the same `isIdle()` and
 *     `hasPendingMessages()` guards.
 *   - OMP's registry exposes `getApiKey(provider)` instead of pi's
 *     `getProviderAuth(provider)`; credentials live in the agent database.
 *   - Default agent dir is `~/.omp/agent` (still overridden by
 *     `PI_CODING_AGENT_DIR`).
 *
 * Resolution order is OpenRouter-only (the operator override for this task;
 * there is no TypeSafe-direct fallback):
 *   1. OMP's native registry auth for provider `openrouter`
...
import { CONTINUATION_QUESTIONS, CONTINUATION_VERSION, NUDGE_MESSAGE } from "./continuation.ts";
...
/** Agent directory: `PI_CODING_AGENT_DIR` or OMP's default. */
export function resolveAgentDir(env: NodeJS.ProcessEnv = process.env, home = homedir()): string {
	return env.PI_CODING_AGENT_DIR?.trim() || join(home, ".omp", "agent");
}
...
interface OmpModelRegistry {
	...
	getProviderAuth?: (provider: string) => Promise<{ auth?: { apiKey?: unknown } } | undefined>;
}

/**
 * Resolve the OpenRouter key. Order: OMP registry, agent-dir auth.json,
...
export async function resolveOpenRouterKey(
	...
	const registry = ctx.modelRegistry as unknown as OmpModelRegistry;
	try {
		const key = await registry?.getApiKey?.("openrouter");
...
export function registerContinuationNudge(pi: ExtensionAPI, deps: ContinuationDeps = defaultDeps): void {
	...
	// OMP has no `agent_settled`. `agent_end` with `willContinue` unset is the
	// terminal settle (the condition OMP's own integrations use); `isIdle()` is
	// false during the event by construction, so the settled event itself is
	// the idle evidence and `hasPendingMessages()` still guards queued work.
