...
| Full compliance in one PR is large, and blocking every PR until the repository is compliant stalls work | Ratchet mode (to build before the next repository). `foundation.json` carries `mode: bootstrap` and a baseline written by `foundation-check baseline`: the current gaps (missing documents, unmapped stories, unwalked stories), each with an owner and an expiry. `check` passes if the gaps are a subset of the baseline and nothing new appears. Any story a PR touches must be mapped and walked, so coverage grows where work happens. An empty baseline flips the mode to `enforced`. |
