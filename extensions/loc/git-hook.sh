...
write_cache() {
	...
	loc_cli="$script_dir/cli.ts"
	agent_dir="${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}"
	if [ ! -f "$loc_cli" ] && [ -f "$agent_dir/extensions/loc/cli.ts" ]; then
		loc_cli="$agent_dir/extensions/loc/cli.ts"
	fi
	if [ ! -f "$loc_cli" ] && [ -f "$ROOT/extensions/loc/cli.ts" ]; then
		loc_cli="$ROOT/extensions/loc/cli.ts"
	fi
	if ! command -v bun >/dev/null 2>&1 || [ ! -f "$loc_cli" ]; then
		return 0
	fi

	payload="$(cd "$ROOT" && bun "$loc_cli" --json --cwd "$ROOT")"
	printf '%s\n' "$payload" >"$TMP"
	mv -f "$TMP" "$CACHE"
