...
`session-close`, `system-one`, `visual-state-review`, and `jev-qa-walk`, plus the
...
| Prompt templates / homebrew skills | have | Portable skills are shared from `agent-config` (ADR-021); pi deploys all 17, with hidden homebrew ones invoked on demand |
