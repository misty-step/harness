...
- **exe.dev mandate:** Heavy execution an agent starts from this workstation (multi-worker suites, full coverage, browser/Electron verification, and long-running services) MUST run in the project's owned exe.dev workspace through `ws` (`skill://using-exe-dev`); CI runners are already off-host. Standing operator approval (2026-09-25) covers one owned `<project>-ws` VM per project within the current $40 exe.dev plan; any other VM requires separate approval. Agent sessions and model credentials remain local until a separate operator decision. Bounded checks with an explicit low concurrency cap (such as `./scripts/verify`), local desktop/GPU work, and data-constrained tasks may run locally.
