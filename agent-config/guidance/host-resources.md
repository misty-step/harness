...
- **Fleet hygiene:** Check for existing runs before starting one; stop only processes owned by this session. Keep one canonical checkout per repository: parallel writers use the harness's native isolated delegation where it exists, and a worktree is only for a separate top-level session that needs its own branch. `git worktree add` and `ssh exe.dev new` in-session take a `session-close.ts add` lease in the same turn; wrap is incomplete until `skill://session-close` exits 0.
