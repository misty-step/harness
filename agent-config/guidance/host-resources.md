...
- **Fleet hygiene:** Check for existing runs before starting one; stop only processes owned by this session. `git worktree add` and `ssh exe.dev new` in-session take a `session-close.ts add` lease in the same turn; wrap is incomplete until `skill://session-close` exits 0.
